...
@testset "SatSet" begin
    @testset "CPro" begin
        mktempdir() do root
            ...
            @test sat.metadata.date.start == DateTime(2012, 2, 5, 21, 16, 31, 602)
            @test sat.metadata.date.stop == DateTime(2012, 2, 5, 22, 55, 12, 809)
            @test all(sat.metadata.granules.latmin .≈ [-81.82188, -68.64167])
            @test all(sat.metadata.granules.latmax .≈ [78.177505, 81.821526])
            @test all(sat.metadata.granules.elonmin .≈ [0.09188683, 20.686209])
            @test all(sat.metadata.granules.elonmax .≈ [44.556667, 179.92279])
            @test all(sat.metadata.granules.wlonmin .≈ [-169.5283, -179.86685])
            @test all(sat.metadata.granules.wlonmax .≈ [-0.17311835, -170.95845])
    ...
    @testset "constructors" begin
        ...
        mktempdir() do root
            ...
            @test sat64.metadata.date.start == DateTime(2012, 2, 5, 21, 16, 31, 602)
            @test sat64.metadata.date.stop == DateTime(2012, 2, 5, 22, 55, 12, 809)
            @test all(isapprox.(sat64.metadata.granules.latmin, [-81.82188, -68.64167]; atol=1e-5))
            @test all(isapprox.(sat64.metadata.granules.latmax, [78.177505, 81.821526]; atol=1e-5))
            @test all(isapprox.(sat64.metadata.granules.elonmin, [0.09188683, 20.686209]; atol=1e-5))
            @test all(isapprox.(sat64.metadata.granules.elonmax, [44.556667, 179.92279]; atol=1e-5))
            @test all(isapprox.(sat64.metadata.granules.wlonmin, [-169.5283, -179.86685]; atol=1e-5))
            @test all(isapprox.(sat64.metadata.granules.wlonmax, [-0.17311835, -170.95845]; atol=1e-5))
...
@testset "Observations" begin
    ...
    @testset "CLay" begin
        ...
        @testset "atmospheric info" begin
            @test TrackMatcher.atmosphericinfo(clay, 16, missing, "test flight") == invalid
            @test (@test_logs(
                (:error, "failed to retrieve atmospheric state for flight test flight at altitude 10000; setting to 'invalid'"),
                TrackMatcher.atmosphericinfo(clay, 42, 10_000, "test flight")
            )) == invalid
            @test TrackMatcher.atmosphericinfo(clay, 16, 13989, "test flight") == ci
            @test TrackMatcher.atmosphericinfo(clay, 16, 13988, "test flight") == invalid
            @test TrackMatcher.atmosphericinfo(clay, 1, 10_000, "test flight") == invalid
            @test TrackMatcher.atmosphericinfo(clay, 1, 40_000, "test flight") == invalid
        end
    end
end
